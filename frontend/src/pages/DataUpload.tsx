...
import type { TrainingClass, TrainingImage, DatasetImageResponse } from '../types';
...
type Tab = 'upload' | 'manage' | 'dataset';
type DatasetTab = 'train' | 'val';
...
export default function DataUpload() {
  ...
  // --- Manage (training_data browse) state ---
  ...
  // --- Dataset (train/val split) state ---
  const [dTab, setDTab] = useState<DatasetTab>('train');
  ...
  const fetchDataset = useCallback(async () => {
    ...
    try {
      ...
      setDError('Failed to load dataset');
    } finally {
      setDLoading(false);
    }
  }, []);
  ...
  useEffect(() => { if (tab === 'manage') fetchBrowseImages(browseFilter); }, [tab, browseFilter, fetchBrowseImages]);
  useEffect(() => { if (tab === 'dataset') fetchDataset(); }, [tab, fetchDataset]);
  ...
  // --- Dataset actions ---
  const currentData = dTab === 'train' ? trainData : valData;
  const dImages = currentData?.images ?? [];
  const isUploaded = currentData?.source === 'training_data';
  const dClasses = currentData?.classes ?? [];
  const imageDir = dTab;

  const handleDatasetDeleteImage = async (filename: string) => {
    setDeleting(true);
    try {
      if (isUploaded) {
  ...
  const handleDatasetDeleteAll = async () => {
    if (isUploaded) {
      toast.error('Uploaded data er jonno Manage tab e delete use koren');
  ...
  const handleDatasetDeleteClass = async () => {
    ...
    <div className="space-y-6">
      <GlassPanel padding={false}>
        <div className="flex border-b border-dark-border overflow-x-auto">
          {(['upload', 'manage', 'dataset'] as Tab[]).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
            ...
            >
              {t === 'upload' ? 'Upload' : t === 'manage' ? 'Manage' : 'Dataset'} {t === 'manage' ? `(${totalImages})` : t === 'dataset' ? `(${(trainData?.count ?? 0) + (valData?.count ?? 0)})` : ''}
            </button>
          ))}
        </div>
      </GlassPanel>
      ...
      {/* ============ DATASET TAB (train/val split) ============ */}
      {tab === 'dataset' && (
        <>
          {dError ? (
            <ErrorMessage message={dError} onRetry={fetchDataset} />
          ) : dLoading ? (
            ...
            <>
              <GlassPanel padding={false}>
                <div className="flex border-b border-dark-border items-center">
                  <div className="flex flex-1 overflow-x-auto">
                    {(['train', 'val'] as DatasetTab[]).map((t) => (
                      <button
                        key={t}
                        onClick={() => setDTab(t)}
                        className={`px-3 sm:px-6 py-2.5 sm:py-3 text-xs sm:text-sm font-medium transition-colors ${
                          dTab === t ? 'text-primary border-b-2 border-primary' : 'text-dark-text hover:text-dark-heading'
                        }`}
                      >
                  ...
                  {!isUploaded && dImages.length > 0 && (
                    <button
                      onClick={() => setConfirmDeleteAll(true)}
                      className="mr-3 flex items-center gap-1 text-[11px] sm:text-xs text-danger px-2 py-1.5 rounded-lg hover:bg-danger/10 transition-colors shrink-0"
                    >
                      ...
                      >
                        ...
                        {dClasses.map((cls) => (
                          <option key={cls} value={cls}>
                            {cls} ({dImages.filter((img) => parseClass(img) === cls).length} img)
                          </option>
                        ))}
                      </select>
                    </div>
              ...
              {dImages.length === 0 ? (
                <EmptyState
                  ...
                  description={`The ${dTab === 'train' ? 'training' : 'validation'} dataset is empty. Upload data first, then start training.`}
                />
              ) : (
                <div className="grid grid-cols-3 xs:grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-7 xl:grid-cols-8 gap-1.5 sm:gap-2">
                  {dImages.map((img) => {
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => confirmTarget && handleDatasetDeleteImage(confirmTarget)}
              disabled={deleting}
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-dark-text">
            ...
            <span className="text-dark-heading font-medium">{dImages.length}</span> image(s) from the{' '}
            <span className="text-dark-heading font-medium">{dTab}</span> dataset? This cannot be undone.
          </p>
          <div className="flex gap-2">
            <button
              onClick={handleDatasetDeleteAll}
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            <button
              onClick={handleDatasetDeleteClass}
